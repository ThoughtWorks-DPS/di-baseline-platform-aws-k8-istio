...
echo "Deleteing A record alias $LOAD_BALANCER_ALIAS_NAME to load balancer $LOAD_BALANCER_HOST_NAME in hosted zone $HOSTED_ZONE_ID|"

cat <<EOF | aws route53 change-resource-record-sets --hosted-zone-id $HOSTED_ZONE_ID --change-batch file:///dev/stdin || true
{
  "Comment": "Deleting A record alias $LOAD_BALANCER_ALIAS_NAME to load balancer $LOAD_BALANCER_HOST_NAME in hosted zone $HOSTED_ZONE_ID",
  "Changes": [
    {
      "Action": "DELETE",
      "ResourceRecordSet": {
